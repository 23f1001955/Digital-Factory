...
def test_gumroad_agent_publish_redirected(tmp_path, monkeypatch):
    from agents import gumroad_agent
    from orchestrator.models import JobSpec, ComponentSpec
    job_spec = JobSpec(slug="test-publish-redirect", product_type="research_pack", niche="test niche")
    comp = ComponentSpec(id="gumroad_publish", agent="gumroad_agent", output="gumroad/published.json", depends_on=[])
    result = gumroad_agent.run(comp, job_spec, {})
    assert result.status == "skipped"
    assert "GumroadChannel" in result.error
...
def test_gumroad_agent_publish_with_delivery_map_redirected(tmp_path, monkeypatch):
    from agents import gumroad_agent
    from orchestrator.models import JobSpec, ComponentSpec
    job_spec = JobSpec(slug="test-publish-dm-redirect", product_type="research_pack", niche="test niche", display_name="Test")
    comp = ComponentSpec(id="gumroad_publish", agent="gumroad_agent", output="gumroad/published.json", depends_on=[])
    result = gumroad_agent.run(comp, job_spec, {})
    assert result.status == "skipped"
