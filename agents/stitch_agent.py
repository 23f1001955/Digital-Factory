...
def _mcp_call(method: str, arguments: dict | None = None) -> dict:
    ...
    resp = httpx.post(STITCH_MCP_URL, json=payload, headers=headers, timeout=MCP_TIMEOUT)
...
def _poll_screen_until_done(project_id: str, screen_name: str, project_title: str) -> dict | None:
    started = time.time()
    while time.time() - started < MAX_POLL_SECONDS:
        try:
            result = _mcp_call("get_screen", {
                "name": screen_name,
        ...
        status = metadata.get("status", "IN_PROGRESS") if isinstance(metadata, dict) else "IN_PROGRESS"
        ...
        if status == "COMPLETE":
            ...
            return screen_obj
        if status == "FAILED":
            msg = metadata.get("statusMessage", "unknown") if isinstance(metadata, dict) else "unknown"
    ...
    logger.warning(f"Screen {screen_name} not ready after {MAX_POLL_SECONDS}s, giving up")
    return None


def _download_screens_from_project(output_base: str, pid: str, project_title: str) -> list[dict]:
    manifest = []
...
def _generate_and_poll_landing(pid: str, prompt: str, project_name: str) -> dict | None:
    ...
    try:
        result = _mcp_call("generate_screen_from_text", {
            "projectId": pid,
            "prompt": prompt,
            "deviceType": "DESKTOP",
        ...
        logger.info("generate_screen_from_text timed out (expected — generation continues in background)")
        result = None
    except Exception as e:
        ...
        for screen in screens:
            ...
            if screen_name and status in ("IN_PROGRESS", "", "UPSCALING"):
                ...
                if screen_id:
                    ...
                    logger.info(f"Polling screen {full_name} (initial status: {status})...")
    ...
    try:
        ...
        if os.getenv("STITCH_API_KEY"):
            try:
                ...
                if project_ids_env:
                    allowed = {pid.strip() for pid in project_ids_env.split(",") if pid.strip()}
                    projects = [p for p in projects if p["name"].split("/")[1] in allowed]
        ...
        if os.getenv("STITCH_API_KEY") and content_summary.strip():
            ...
            try:
                ...
                if not pid:
                    logger.warning("Failed to create Stitch project for runtime landing page")
                else:
                    ...
                    screen = _generate_and_poll_landing(pid, landing_prompt, project_name)
                    if screen:
                        safe_name = screen.get("title", "Landing_Page").replace(" ", "_").replace("/", "_")
                        html_info = screen.get("htmlCode", {})
                        html_url = html_info.get("downloadUrl")
                        html_path = None
                        if html_url and html_url != "UPLOADING":
                            ...
                            ok = _download_file(html_url, html_path)
                            if ok:
                                logger.info(f"Landing page HTML from Stitch: {html_path}")
                        ...
                        if img_url:
                            ...
                            ok = _download_file(img_url, img_path)
                            if ok:
                                logger.info(f"Landing page image from Stitch: {img_path}")
        ...
        if landing_page_html:
            ...
            logger.info("No Stitch landing page created — landing_agent will use LLM fallback")
